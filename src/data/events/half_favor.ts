...
export const halfFavorEvents: Record<string, GameEvent> = {
  ...
  half_favor_lin_xiaojuan: {
    ...
    description: [
      ...
      '「你他妈的为什么又是NaN。」他眉头皱着，语气介于恳求和指控之间，「你上午还好好的，你背叛我了吗。」停顿了两秒，继续：「你把梯度藏哪里了？你说！」',
      '他完全没注意到你站在门口。你看了大概十秒，发现他不只是在骂——他骂完之后会停下来认真看报错信息，骂完再看，骂完再看，像是把情绪发泄和debug同步进行，效率奇高。',
    ],
    prompt: '你要不要出声？',
    options: [
      {
  ...
  half_favor_gu_mianmian: {
    ...
    description: [
      ...
      '全场安静了两秒。你顺着想了想，确实是。大家刚想说什么，转头看——她已经真的睡着了，头轻轻靠在手背上。',
    ],
  ...
  half_favor_ye_zhiqiu: {
    ...
    description: [
      ...
      '你说这只是宣传语，她点了点头，「我知道，」停顿两秒，「但如果你有四道招牌，就意味着你没有招牌。」',
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '{studentName}立刻拿回菜单，开始重新规划布局，说了将近七分钟，逻辑严密没有废话，最后给出「用户体验最优解」。你发现她在这件事上消耗的精力，可能比点餐本身多二十倍。但她讲完之后，表情是满足的。饭点到了，她看了一眼手机，「我们应该先点餐了。」',
          effects: [
            { type: 'randomStudent', stat: 'happiness', delta: 15 },
            { type: 'randomStudent', stat: 'skills.theory', delta: 3 },
          ],
      ...
      },
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '{studentName}认真想了一下，「理论上，我可以提供菜单设计咨询，但他们可能不会付钱。」你说大概真的不会，她点头，「那算了。」然后继续翻菜单。过了约五秒，她又开口：「这里的蛋白质和碳水配比也不太合理。」你放弃了继续反应。',
          effects: [
            { type: 'randomStudent', stat: 'happiness', delta: 12 },
            { type: 'randomStudent', stat: 'skills.social', delta: 3 },
          ],
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '{studentName}面无表情地打开手机，三十秒后向你展示了一个表格，列了附近十二家餐厅，标注了人均价格、距离、等位时间和组内已知偏好匹配度。「我有时候会顺手建这个，」他说，语气一本正经，「只是顺手。」你问顺手有多久了，他说大概一年。整个实验室的午饭开销，可能从未如此被认真对待过。',
          effects: [
            { type: 'lab', stat: 'reputation', delta: 1 },
            { type: 'randomStudent', stat: 'happiness', delta: 12 },
            { type: 'randomStudent', stat: 'skills.social', delta: 3 },
  ...
  half_favor_he_shixu: {
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '{studentName}停顿了一秒，然后平静地点头，「每天一条，执行了快两年了。」她把平板收起来，「研究发现，及时的正向反馈可以显著提升团队效率。」她又补了一句，声音比刚才小了一点：「而且，很多人做了很多事，但没有人告诉他们。」你想接话，她已经站起来去拿文件了。',
          effects: [
            { type: 'randomStudent', stat: 'happiness', delta: 12 },
            { type: 'randomStudent', stat: 'skills.social', delta: 3 },
          ],
  ...
  half_favor_mo_wenxuan: {
    ...
    description: [
      ...
      '某次你忍不住问了。他完全没有不好意思，认真地解释：「改变一个变量，排除椅子高度对结果的影响。」你盯着他看了三秒，他补充：「我知道这不科学，但心情会影响操作质量，操作质量会影响结果。所以我需要一个信号，告诉自己：现在认真了。椅子比较方便。」',
    ],
    prompt: '你：',
    options: [
      {
