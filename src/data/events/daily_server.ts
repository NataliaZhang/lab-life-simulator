    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你决定采取"战略忽视"策略。进程在第五天跑完了，GPU安静地归还给了队列。没有人承认，没有人解释，生活继续。但群里从那天起多了一条不成文的规定：进程名必须包含自己的名字缩写，没有人公开提出这条规定，但大家都开始遵守了。',
            effects: [
      ...
      },
      {
        ...
        outcomes: [
          ...
          {
            weight: 1,
            narrative: '消息发出去了，收到了三条回复。第一条："不是我。" 第二条："不是我。" 第三条："我也在等GPU，都等两天了。" 进程没有消失，神秘仍在继续，但你成功地把实验室氛围从"各自刷手机装没看见"升级到了"主动互相指认"。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -5 },
            ],
          },
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你发了一条消息："更新了就更新了，等它自己重启，活了皆大欢喜，不活我们修。" 群里有人回复了一个"……"，有人发了一张正在叩头的表情包。最终服务器活了，但它花了十九分钟才完成重启，这十九分钟实验室里没有人说话，所有人都在盯着自己的屏幕等待。1127天的uptime就这样归零了，大家盯着各自屏幕的神情，有一种见证历史落幕时那种说不出来的低气压。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: 3 },
              { type: 'lab', stat: 'energy', delta: 5 },
            ],
          ...
          {
            weight: 1,
            narrative: '你发了消息说等它重启，然后去接了杯水。回来的时候，有人已经在群里贴出了一份完整的"服务器死亡预案"文档，包括数据备份地址、临时迁移方案和三台备用服务器的SSH密钥。原来每个人心里都有一个这个文档，只是平时不说出来。服务器最终活了，文档存进了组里的confluence，这是这件事唯一的正面遗产。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: 5 },
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你宣布：即日起每人/data配额100GB，超额文件72小时内自行处理，否则系统自动归档到冷存储。群里先是有人回了个句号，过了三分钟才有人问"checkpoint算不算"，你说算。接下来的两天组里进行了一次大规模的哲学反思：什么是"真正需要的"权重文件，什么只是"心理安慰"。最终清出了700GB，磁盘使用率降到了58%。代价是有三个人找你单独谈了一次"能不能额外申请一点空间"。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -10 },
              { type: 'allStudents', stat: 'favor', delta: -5 },
              { type: 'lab', stat: 'energy', delta: -10 },
      ...
      },
      {
        ...
        outcomes: [
          ...
          {
            weight: 1,
            narrative: '你买了硬盘，挂载的时候发现文件系统格式不兼容，重新格式化又花了一个上午。在这段时间里，/data真的写满了，有三个人的训练任务因为"no space left on device"崩掉了。硬盘最终挂好了，但今天的战损已无法挽回。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -5 },
      ...
      },
      {
        ...
        outcomes: [
          ...
          {
            weight: 1,
            narrative: '你宣布了大赦令，然后等待。48小时过去了，磁盘释放了……37GB。大家都"清理了一下"，但每个人内心深处都认为自己的那份checkpoint才是真正的科研遗产。你叹了口气，开始思考是该立威还是该买硬盘的哲学问题。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: 3 },
              { type: 'lab', stat: 'energy', delta: -10 },
            ],
    ...
    options: [
      {
        ...
        outcomes: [
          ...
          {
            weight: 1,
            narrative: '你组织了重建，发现大家的requirements.txt都是不同时期、不同状态的快照，有的有，有的没有，有的最后更新时间是2022年。最终花了整整两天才让所有人回到可以正常跑代码的状态。在这两天里，整个组的科研进度集体停摆，像一台被人拔了电源的机器。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -15 },
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你发了条消息："依赖更新了，有问题的自己用pip install --upgrade修一下，装新包记得用conda。" 接下来一周，组里每天都有新的依赖冲突冒出来，每个人都独立解决了自己的问题，由此意外积累了相当水平的包管理实战经验。科研延误了，但engineering skill集体长进了，这算是某种意义上的因祸得福。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -8 },
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你在群里发了紧急通知，两个人立刻在训练脚本里插入了checkpoint保存逻辑，在凌晨1:47完成了保存，距离重启还有13分钟。损失从"重新从12小时前开始"压缩到了"从1小时前开始"。事后整个组更新了训练脚本模板，加入了自动定时checkpoint，频率：每两小时。这是一次价格合理的教训。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -3 },
              { type: 'allStudents', stat: 'skills.engineering', delta: 5 },
            ],
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你在群里发了一条消息："重启不可避免，今晚的训练会中断，重新跑就是了。大家记得下次脚本里加上定时checkpoint。" 两个正在跑任务的人分别回了"好的"和一个点赞，没有问号，没有感叹号，大概是已经经历过了足够多次，打字时手都不会抖了。凌晨两点，服务器准时重启，组里所有人都睡着了，唯独IT的运维日志里多了一条成功记录。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -8 },
              { type: 'allStudents', stat: 'skills.engineering', delta: 3 },
              { type: 'lab', stat: 'energy', delta: 5 },
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          ...
          {
            weight: 1,
            narrative: '你们清除了进程，打了补丁，以为万事大吉。两天后，IT发来了第二份报告：同一IP，换了一个端口，矿机重新上线了，因为打补丁的时候漏掉了一个子进程。这次你上报了校方，并在内心为那个坚持不懈的矿机作者打了一个低分的技术致敬。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -8 },
              { type: 'lab', stat: 'energy', delta: -15 },
            ],
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你发了通知：本周五三个老环境全部删除，届时会有新的lab_env_2024，带锁定版本的requirements.txt，有需求提前说。过了两天，有人来找你说老环境里有一个自己魔改的包，问能不能备份一下。你问他是哪个包，他说他忘了。核爆准时进行，新环境在周五下午建好，整洁干净，像一个新实验室。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -5 },
              { type: 'allStudents', stat: 'skills.engineering', delta: 5 },
              { type: 'lab', stat: 'energy', delta: -15 },
          ...
          {
            weight: 1,
            narrative: '你执行了核弹，删了三个环境，建了新的。第二天有人来说他的实验脚本在新环境里报错，定位发现是一个三年前的遗留依赖，版本号已经从PyPI上消失了。你花了一个上午从conda-forge里找到了等价的替代方案，感觉自己在做考古挖掘。新环境最终可用了，但代价是实验室所有人都意识到：之前那三个混乱的环境，其实在各自承载着不同的历史遗迹。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -10 },
              { type: 'allStudents', stat: 'skills.engineering', delta: 8 },
              { type: 'lab', stat: 'energy', delta: -20 },
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你花了一个下午pip list了三个环境，做了对比分析。发现lab_env是稳定的"跑得通但版本旧"版本，lab_env_v2是有人升级了torch之后顺带出问题的中间态，lab_env_new是为了解决lab_env_v2的问题而建的临时版本，结果解决了torch的问题，又引入了一个新的CUDA兼容问题。你把这份历史写成了两页文档，存进了组里的wiki。读完这份文档，大家都对这三个环境产生了一种奇异的理解与同情。',
            effects: [
              { type: 'allStudents', stat: 'skills.engineering', delta: 8 },
              { type: 'lab', stat: 'energy', delta: -15 },
            ],
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你宣布新建lab_env_official，这是官方版本，有文档，有requirements.txt，锁定了所有关键版本。消息发出三天后，conda env list里出现了四个条目。lab_env_official旁边，三个旧环境纹丝未动。大家对于"迁移到新环境"这件事持续观望，理由是"等我这个实验跑完"，而这个实验似乎永远处于"快跑完了"的状态。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -3 },
              { type: 'lab', stat: 'energy', delta: -10 },
            ],
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你给IT主管发了邮件，附上了受影响的用户数量（11人）、每日科研工时估计（约82小时）和权限丢失的具体时间线，措辞客观，数据详实，语气没有任何情绪，但事实本身就已经足够有力。IT主管在两小时内回复，表示下午三点前会恢复。三点零七分，home目录权限回来了。这份邮件后来被同组同学截图保存，作为"如何有效与IT沟通"的范本。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: 10 },
              { type: 'lab', stat: 'energy', delta: -10 },
              { type: 'lab', stat: 'reputation', delta: 2 },
          ...
          {
            weight: 1,
            narrative: '你升级了投诉，IT主管回复说"正在内部协调"。又过了一天，权限依然只读，但你收到了一封措辞非常正式的道歉邮件，说明了问题的技术原因，并承诺周三上午十点前恢复。周三上午十一点，恢复了。这比"本周内"快了很多，但是比你预期的晚了一个小时。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: 3 },
              { type: 'lab', stat: 'energy', delta: -15 },
            ],
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你在群里发了一条消息："今天home目录只读，改代码不方便，大家趁机整理一下文档、写写论文、看看文献。" 出人意料的是，这个星期大家的写作产出是过去一个月里最高的。代码跑不了，逼出了久拖未决的related work。有一个学生说"老师其实可以每周停一天服务器"，你没有评论这个建议，但把它记在了心里。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: 8 },
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你在组里宣布了新规定：所有训练脚本必须包含每两小时一次的checkpoint保存，不符合要求的脚本不允许在服务器上提交。同时提供了一个模板wrapper，三行代码可以接进任何训练循环。这次停电损失了35小时，但这条规定从此把所有未来的意外损失上限压到了2小时。这是今天最有价值的决策，尽管它无法挽回已经消失的14000块钱。',
            effects: [
              { type: 'allStudents', stat: 'skills.engineering', delta: 8 },
              { type: 'allStudents', stat: 'happiness', delta: -3 },
              { type: 'lab', stat: 'energy', delta: -10 },
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你清出了磁盘空间，通知大家重新提交实验。有人说"那六天不是白跑了吗"，你说"至少验证了GPU是好的"。这不是一句有用的话，但是一句能让人继续往下走的话，而有时候这就是全部所需。实验在当天重新排队，组里的情绪像一台被强行重启的系统：有些程序加载慢了，但总体上还在运行。',
            effects: [
              { type: 'allStudents', stat: 'happiness', delta: -12 },
      ...
      },
      {
        ...
        outcomes: [
          ...
          {
            weight: 1,
            narrative: '你配置了监控，设置了80%时发报警邮件。三天后，你的邮件收件箱里出现了第一条告警："磁盘使用率已达81%"。这是因为你刚才清磁盘之后，大家重新提交了所有实验，六天的补跑产生了大量输出。报警系统完美运作，成功地在磁盘再次接近满时通知了你，这是一种令人略感挫败的成功。',
            effects: [
              { type: 'allStudents', stat: 'skills.engineering', delta: 5 },
              { type: 'allStudents', stat: 'happiness', delta: -8 },
              { type: 'lab', stat: 'energy', delta: -15 },
