...
export const reviewerEvents: Record<string, GameEvent> = {
  ...
  reviewer_agi_prerequisite: {
    ...
    title: '先解决AGI再来投稿',
    description: [
      'ICOP的审稿意见到了。审稿人一号给了条件接受，审稿人三号简短但友善。',
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你把这条审稿意见截图保存到了专用文件夹，命名为"2号经典收藏"，然后关掉投稿系统，打开新的投稿目标。改投了IISC，两个月后论文过了，审稿人一号写道："本文思路清晰，实验充分。"你对着屏幕点了点头，感觉世界大体上是公平的，只是有些会议活在另一个物理层上。',
            effects: [
          ...
          {
            weight: 1,
            narrative: '你换投了GRF。GRF的审稿人三号写道："本文方法有效，但缺乏对更广泛场景的泛化能力的讨论，在极端情况下，是否需要更强的推理能力？"你盯着这条意见看了很久，感觉审稿人二号换了一个马甲，跟着你跑到了另一个会。',
            effects: [
  ...
  reviewer_ghost: {
    ...
    title: '五个月过去了，什么都没有',
    description: [
      '投稿系统显示：Under Review。你往OWRC投的稿已经保持这个状态整整五个月，审稿意见一条都没有出现。',
      '你发了三封询问邮件，措辞依次从客气到更客气。AC没有回复。你去查了一下，发现OWRC的主会场已经在两个月前开完了，论文集已经上线，但你的稿件既不在接受列表里，也不在拒稿列表里。',
  ...
  reviewer_scope_again: {
    ...
    description: [
      ...
      '这是第四个会议说出这句话。第一个是IISC，第二个是HMCS，第三个是OWRC，第四个的主题是"AI与现实世界"，范围之广在召集书里写得相当清楚。',
  ...
  reviewer_insider: {
    ...
    description: [
      ...
      '你重新读了一遍。那个消融实验，只存在于你内部的文件里，从未对外公开。采样策略的具体细节，应该只有组里的人知道才对。',
  ...
  reviewer_format_rejection: {
    ...
    title: '摘要字号差了一号',
    description: [
      'HMCS发来desk rejection通知。理由："根据本会议投稿要求，摘要部分应使用10pt字体，作者提交版本使用了11pt字体，不符合格式规定，故不予送审。"',
      '摘要一共只有四行。字号差了一号。你顺手翻了一下HMCS的格式要求文档，发现第七条规定摘要必须使用10pt字体，第十一条规定伦理声明不得少于200字，第十二条规定伦理声明模板不可修改。',
    ],
  ...
  reviewer_retroactive_scoop: {
    ...
    description: [
      ...
      '你去查了那篇引用。它是昨天下午三点整才上传到arxiv的。而这是你六个月前投进GRF的稿件。',
  ...
  desk_reject_48h: {
    ...
    title: '周三早上，拒稿了',
    description: [
      '周一往ICOP投了一篇稿。周三早上，投稿系统发来邮件："我们遗憾地通知您，经过编辑初审，您的论文与本刊的发表范围不够契合，因此无法进入同行评审流程。"',
      '没有任何理由。你曾在ICOP发过两篇论文——其中一篇，是口头报告。',
    ],
