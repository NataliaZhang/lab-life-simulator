...
export const xieZhiweiEvents: Record<string, GameEvent> = {
  ...
  xzw_first_meeting: {
    ...
    title: '我就看看',
    description: [
      '你去楼下接谢知微，发现她蹲在门口的网络设备柜前，像小动物发现了会发光的洞口，半个脑袋都快探进去了。"这个型号我查过，"她抬头冲你眨了眨眼，手指还恋恋不舍地搭在柜门边上，"我就想看看它里面怎么挂载配置的，真的不动。"保安大叔抱着保温杯，表情像是在判断要不要报警。',
      '进了办公室，谢知微刚坐下，眼神就被你的电脑屏幕吸住了。她努力把手放到膝盖上，指尖却自己动了动。"老师，"谢知微小声说，"你的浏览器好像快被标签页淹没了。"她又补了一句，语气特别诚恳："我可以只看一眼，不清也行。"',
    ],
    prompt: '你怎么回应她？',
    triggerConditions: [
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '谢知微立刻坐直，把小本子翻开，笔帽咔哒一声扣好。"好，我们说正事。"她看起来乖得很，只有视线偶尔从本子边缘溜出去，碰到屏幕又立刻缩回来。你讲到项目安排时，她在纸角悄悄画了一个小小的浏览器图标，又很快用手掌盖住。',
            effects: [
  ...
  xzw_just_look: {
    ...
    title: '顺手重构了一下',
    description: [
    '服务器日志格式乱掉了，你让谢知微"帮忙看一眼"。这是你今天说过的最危险的话。',
    '过了一阵，谢知微发来消息："问题解决了！另外，日志系统我也顺手理了一下。"后面跟着一个乖巧的笑脸。',
  ...
  xzw_accidentally_broke: {
    ...
    title: '原来的版本还在吗',
    description: [
      '谢知微把训练脚本优化了，跑得明显快了，发消息时还带着一串小烟花表情。你也挺高兴，直到你发现新脚本的checkpoint格式变了。',
      '旧结果读不进来了。',
      '你问谢知微怎么回事。她那边很快弹出"正在输入"，又消失，又弹出来。最后发来一句："老师，原来的版本……还在吗？"后面跟了一个小心探头的表情。',
    ],
    prompt: '你怎么处理这件事？',
    triggerConditions: [
      { type: 'time', field: 'year', op: '>=', value: 1 },
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 3,
            narrative: '谢知微抱着电脑连连点头，头发上的小发卡都跟着晃。"我知道了，我现在救。"她当天写了格式转换脚本，把大部分旧结果捞了回来。最后她把脚本推上去，附带一个兼容性检查，还在说明里认真写道：以后改格式前先问老师，不可以手痒。你看着这行字，不确定这条规则是写给自己看的，还是写给你看的。',
            effects: [
          ...
          {
            weight: 1,
            narrative: '谢知微立刻开始补救，键盘敲得像小仓鼠在抢修水管。可惜旧格式比想象中更乱，最后只救回了一部分。她把失败样例整理出来，给每一种都标了原因，末尾还加了一条新的仓库规则：任何格式变更必须附转换脚本。她看着规则，轻轻戳了戳屏幕："这条是给我自己的。"',
            effects: [
  ...
  xzw_elegant_solution: {
    ...
    title: '优雅',
    description: [
      '没有人给谢知微安排这个任务。她只是路过一个核心模块，点进去看了看，然后就没能忍住。',
      '等你发现时，模块已经被谢知微重新整理了一遍：代码更短，跑得更快，测试也补齐了。她把PR标题写得很无辜：小幅整理。',
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '谢知微抱着电脑上去讲，一开始还把投影线插反了，自己低头笑了一下。讲到代码结构时，她整个人立刻亮起来，从设计决策讲到测试覆盖，越讲越顺。有人问她为什么想到这样拆模块，她认真回答："因为它们本来就不该挤在一起。"全组听完，默默打开了自己的旧代码。',
            effects: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '"那个已经做完啦。"谢知微把主线结果递给你，又把重构PR推过来，像小朋友同时交出作业和手工。"这个是等实验的时候做的，不耽误。"你看着她亮晶晶的眼神，意识到对她来说，别人刷手机的碎片时间，她用来给代码做大扫除。',
            effects: [
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '"好。"谢知微乖乖把螺丝刀放回去，还用两根手指把它推远了一点，表示自己真的没有动手。然后她绕着设备转了一圈，又蹲下来看散热口。"我不拆，我用眼睛拆。"你决定暂时相信这句话，同时把工具箱挪到了更高的柜子里。',
            effects: [
              { type: 'student', studentId: 'xie_zhiwei', stat: 'favor', delta: 3 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'happiness', delta: -2 },
            ],
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '"真的？"谢知微立刻拿出手机，把保修到期日记进日历，事件标题写得非常朴素：可以拆。你看着她认真设置提醒，忽然觉得这台设备的未来已经被安排好了。她收起手机，心满意足地拍了拍纸箱："你先好好工作，之后我再来认识你。"',
            effects: [
              { type: 'student', studentId: 'xie_zhiwei', stat: 'favor', delta: 6 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'happiness', delta: 4 },
            ],
  ...
  xzw_optimization_spree: {
    ...
    title: '顺手优化了五个',
    description: [
      '这周你没给谢知微额外任务，她应该在推进自己的研究。周五下午，她发来一份清单。',
      '实验室几个常用工具，全被她摸过一遍。每个后面都有benchmark、改动说明和一小段风险提示。',
      '"跑实验的时候手边空着。"谢知微解释得很自然，"然后我看见有个循环可以少绕一下。改完这个，又发现旁边那个也有点别扭。"她发来一个不好意思的表情："就一个接一个了。"',
    ],
    prompt: '你怎么回应这场优化风暴？',
    triggerConditions: [
      { type: 'time', field: 'year', op: '>=', value: 2 },
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '谢知微去了。二十分钟后回来，bug在。她还带回来一份复现步骤文档和一个修复 patch，格式规范得像交作业。"我怕自己直接改会顺手改多，先来确认一下。"她说，手在键盘边缘停着，等你点头。\n\n你点头。她才放下来，开始敲。\n\n你后来想，如果没让她先复现，那个 patch 后面可能会附一个"顺手把旁边的也一起改了"。',
            effects: [
              { type: 'student', studentId: 'xie_zhiwei', stat: 'favor', delta: 5 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'happiness', delta: 4 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'skills.engineering', delta: 4 },
  ...
  xzw_version_control: {
    ...
    title: '我顺手把历史整理了一下',
    description: [
      '你让谢知微修一个小bug，预计很快就能结束。',
      '她发来pull request：bug修好了。附带说明："我顺手把仓库历史收拾了一下，还写了一个贡献指南。"',
      '你打开仓库，发现废弃分支少了一大片，README旁边多了CONTRIBUTING.md。谢知微在PR里补了一句："删之前都有备份！这次我记得了！"',
    ],
    prompt: '你怎么回应这次"顺手"？',
    triggerConditions: [
      { type: 'time', field: 'year', op: '>=', value: 2 },
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '"对吧！"谢知微一下来了精神，开始解释乱分支以后会怎样干扰排查，还把贡献指南里的几条规则讲给你听。她讲得很认真，像在介绍自己刚整理好的抽屉。你把文档发给全组，标题写：以后按这个来。她看到后，偷偷在文档末尾加了一句：如需修改，欢迎开issue。',
            effects: [
  ...
  xzw_accidental_discovery: {
    ...
    title: '我只是看看别人怎么做的',
    description: [
      '谢知微发来一页技术备忘录，标题是："可能有用，也可能只是我想多了。"',
      '内容写得很细：谢知微在看另一个领域的实现时，发现他们处理某类数据的方式和你们当前瓶颈很像。她整理了原理、适用条件，以及迁移过来大概要改哪些地方。',
      '"我真的只是看看别人怎么做的。"谢知微解释，"然后它看起来一直在向我招手。"',
    ],
    prompt: '你怎么处理这个意外发现？',
    triggerConditions: [
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 2,
            narrative: '你把备忘录发给组里讨论，大家越看越觉得有戏。你告诉谢知微这个方向值得试时，她眼睛慢慢睁大："真的有用啊？"她不是装谦虚，她确实只是追着好奇心摸过去，没想到摸到了一条新路。后来这个想法帮你们省掉了不少试错时间，她也从此更理直气壮地"只是看看"了。',
            effects: [
              { type: 'student', studentId: 'xie_zhiwei', stat: 'favor', delta: 8 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'happiness', delta: 5 },
          ...
          {
            weight: 1,
            narrative: '你认真看完，发现几个关键假设在你们这里不成立。你把原因讲给谢知微，她立刻把备忘录改名成"不适用但学到了"，还在旁边标注了失败原因。"那也不亏。"她说，顺手把相关资料归档进了组里的知识库。这个方向没走通，但下次有人问起，她已经把坑画好了。',
            effects: [
              { type: 'student', studentId: 'xie_zhiwei', stat: 'favor', delta: 6 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'happiness', delta: 4 },
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '"好，那我先存起来。"谢知微给备忘录加上"待评估"标签，还贴心地写了摘要，方便以后捡回来。后来事情一件接一件，那份文档一直躺在角落。等你在别人的论文里看到类似思路时，脑子里冒出的第一句话是：谢知微当时好像真的捡到过这块石头。',
            effects: [
              { type: 'student', studentId: 'xie_zhiwei', stat: 'favor', delta: 2 },
              { type: 'student', studentId: 'xie_zhiwei', stat: 'happiness', delta: -2 },
            ],
  ...
  xzw_alumni_visit: {
    ...
    description: [
      ...
      '邮件末尾说：谢知微整理了一套开源工具，想以实验室名义一起发布。她说这些工具有一部分是在这里做研究时攒下来的，想还给这里。',
    ],
    prompt: '你怎么回复谢知微？',
    options: [
      {
