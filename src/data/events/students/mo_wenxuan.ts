...
export const moWenxuanEvents: Record<string, GameEvent> = {
  ...
  mwx_first_meeting: {
    ...
    title: '先调一下气场',
    description: [
      '莫问玄进门，先没急着坐，而是以一种职业性的眼神扫了一圈你的办公室：书架、窗、桌面，像在做系统诊断，缺的不是数据，是风水数据。',
  ...
  mwx_tarot_before_submit: {
    ...
    title: '今天宜投稿',
    description: [
      '论文改到位了，你们约好今天提交。莫问玄准时出现在走廊，但没有立刻进来，隔着玻璃，你看见他从书包侧袋摸出一副牌，在走廊灯光下认真地抽了一张，对着灯举起来看了好一会儿，表情严肃得像在读PCR结果。',
  ...
  mwx_lucky_timing: {
    ...
    title: '玄学成功率：68%',
    description: [
      '上次投稿，莫问玄通过占卜选了一个"宜提交"的日子，比截止日期早了整整四天。你当时只是让他不要拖就好，没有特别在意那个日子的玄学属性。',
  ...
  mwx_gpu_fengshui: {
    ...
    description: [
      ...
      '莫问玄在白板前分析了整整一个下午，把所有工程原因列成一排：内存泄漏、CUDA版本冲突、数据加载并发、驱动兼容性……逐条测试，逐条排除，排除完他在白板上画了一个箭头，指向机房方向。',
  ...
  mwx_interview_ritual: {
    ...
    title: '幸运蓝T恤出战',
    description: [
      '莫问玄去某大厂实习面试，出发前给你发了一条消息，语气轻快："老师，今天大吉，运势：事业宫旺，利于面试，忌犹豫。准备完毕，出发。"',
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '"实力当然够，我准备了三周，把常见题型都过了一遍，项目经历整理得清清楚楚。幸运蓝的作用是走进面试间那一秒不紧张。"他发来他的面试准备清单，密密麻麻，一点都不像靠运气的人。\n\n他最后说："玄学不是实力的替代品，是催化剂，让实力以最大效率释放。"你读完这句话，觉得这个学生将来大概会去写自我效能感的商业咨询报告。',
            effects: [
  ...
  mwx_bad_omen: {
    ...
    title: '这个月时机不对',
    description: [
      '你们在讨论要不要赶一个月底的会议截止，莫问玄来找你，表情比平时认真了一个刻度，说："老师，我测算了一下，这个月时机不太对，我们这个成果，不建议投这个会议。"',
      '莫问玄列了几条维度：这个会议的接受率历年来在该月份偏低，组里这段时间修改状态不够稳定，还有一个他说"难以量化但直觉显著"的整体气场判断。',
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你说"好，这个会议先缓，下一个窗口再看"，他点头，说"下个季度气场更顺"。\n\n后来那篇论文投进了下一个会议，进了一个更匹配的评审委员会，结果反而更好。\n\n他在成功率统计里把这条记成了"建议被采纳，结果正面"，成功率那栏打了个勾，旁边写：双因子对齐（玄学+时间管理）。',
            effects: [
              { type: 'student', studentId: 'mo_wenxuan', stat: 'favor', delta: 7 },
              { type: 'student', studentId: 'mo_wenxuan', stat: 'happiness', delta: 6 },
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '他点头，无缝切换叙述模式，从气场档换到实际分析档：审稿周期在当前修改进度下偏紧，接受率这两年下降，建议把精力集中到另一个更匹配的会议。\n\n你听完觉得这个判断本身是合理的，同意先缓一缓。他最后补了一句："玄学结论和实际结论这次一致，效率高了。"\n\n你不确定他在夸玄学还是夸自己的分析，或者在他那里这两件事本来就没有区别。',
            effects: [
      ...
      {
        id: 'mwx_bad_omen_override',
        text: '玄学归玄学，会议还是要投',
        outcomes: [
          {
            weight: 1,
            narrative: '你说"截止日期就是截止日期，这个会我们投"。他停顿了一秒，点头，没有再说什么。\n\n接下来那段时间，基金申请和论文修改的截止日正好重叠，组里连着几天几乎没有整块时间——就和你早就预料到的一样，就和他说的一样，只是他不知道你早就知道。\n\n论文在最后几个小时赶完提交，格式错误修了两遍，你自己也改到了凌晨。\n\n他在成功率统计里记下了这条："建议被否决。双线任务冲突，截止日重叠，能耗超出预期。结果待定。" 备注栏里加了一行："气场判断得到实证支持，但未被采纳，数据价值存疑。"',
            effects: [
              { type: 'student', studentId: 'mo_wenxuan', stat: 'favor', delta: -5 },
              { type: 'student', studentId: 'mo_wenxuan', stat: 'happiness', delta: -6 },
  ...
  mwx_statistics: {
    ...
    description: [
      '莫问玄发来一份PDF，标题是《个人决策辅助系统效果评估报告（第一年度）》，共十一页。',
      '报告统计了过去一年内所有涉及玄学参考的决策，共43条，分"遵循玄学建议"与"忽略玄学建议"两组，记录每条的背景、结果，以及莫问玄认为的成功/失败判定标准。',
      '结论部分写着：遵循组成功率68%，忽略组46%，差值22个百分点，p=0.031，95%置信区间[0.04, 0.40]。下面附了一行小字：样本量偏小，结论应谨慎解读；混淆变量（心理状态、执行质量）尚未完全控制。莫问玄自己写了局限性，措辞和他投过的论文里用的一模一样。',
    ],
    prompt: '你怎么回应莫问玄这份报告？',
    triggerConditions: [
      { type: 'time', field: 'year', op: '>=', value: 2 },
  ...
  mwx_skeptic_debate: {
    ...
    title: '影响的是心理状态',
    description: [
      '实验室来了一位访学研究员，性格直接，在组会茶歇听说了莫问玄塔罗投稿的事，当场发问："你是认真相信这个的吗？这不是典型的确认偏误？"',
      '气氛安静了片刻。莫问玄把茶杯放下，没有表现出任何被冒犯的迹象，开口，语气平静到像在组会上讲方法论：',
  ...
  mwx_paper_title_omen: {
    ...
    title: '标题笔画数的问题',
    description: [
      '论文初稿成形，你们在敲定标题。莫问玄对着当前版本看了一会儿，翻出手机，算了一下，抬头说："老师，这个标题有几处笔画数加起来……不太顺，有点散。我想改一下。"',
      '莫问玄把要改的位置指给你看，说出替换方案。你把两个版本都念了一下，发现改完的标题确实更简洁：字数少了一个，节奏更紧凑，读起来更有力。只是他本人的出发点和这些优点没有任何关系。',
    ...
    options: [
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '你说"这个版本更简洁，改吧"，莫问玄更新了文档，说"好"，若无其事地继续讨论摘要。你注意到他没再提笔画数的事，像是在一张内部清单上打完了勾，可以继续下一项。论文投出去后，一个审稿人在评语里专门写标题"精确而简洁"。他看到这条，给你发了一条消息，只有一句：笔画数：有效。',
            effects: [
              { type: 'student', studentId: 'mo_wenxuan', stat: 'favor', delta: 8 },
              { type: 'student', studentId: 'mo_wenxuan', stat: 'happiness', delta: 8 },
      ...
      },
      {
        ...
        outcomes: [
          {
            weight: 1,
            narrative: '莫问玄点头，把标题搁下，认真讨论摘要去了。你注意到他在笔记本上把那个改动记了下来，旁边写了一行小字：待定，内容优先。两周后做最终审阅，他翻到那页笔记，说："老师，之前那个标题，可以定了。"你看了一眼，觉得改完的确实更好，同意了。他在成功率统计里把这条记成了"延迟采纳，结果正面"，并加了备注：说明玄学建议具有时间稳健性。',
            effects: [
              { type: 'student', studentId: 'mo_wenxuan', stat: 'favor', delta: 6 },
              { type: 'student', studentId: 'mo_wenxuan', stat: 'happiness', delta: 5 },
              { type: 'student', studentId: 'mo_wenxuan', stat: 'projectProgress', delta: 3 },
  ...
  mwx_alumni_visit: {
    ...
    description: [
      ...
      '毕业之后莫问玄去了一家AI公司。入职没多久，他在团队里推动了一套"发布前玄学评估流程"：每次重大版本上线前，有一个非强制性的环节，让有意愿的同事抽一张塔罗牌，记录感受，然后继续正常的技术评审。',
