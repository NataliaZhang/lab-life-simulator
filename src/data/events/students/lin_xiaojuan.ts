...
export const linXiaojuanEvents: Record<string, GameEvent> = {
  ...
  lxj_first_meeting: {
    ...
    description: [
      ...
      '他把奶茶放在你桌对面，拉开椅子，以一种叫人无从发火的镇定坐了下来，然后用聊天气的语气开口："老师好，我是林小卷，新来的，以后多关照。" 停顿一秒，把奶茶往前推了推："这家真不错，要不要来一口？"',
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你发消息："这次完成得相当好。" 五分钟后收到回复：一个大拇指表情，然后是"谢谢老师，我去睡了"。接下来两天他彻底消失，据室友说一觉睡了十七小时，打破了实验室纪录——那个纪录之前也是他自己创下的。传奇打破传奇，闭环完美。',
          effects: [
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你放下手机，打开电脑，把他那团思绪仔细梳理了一遍，发现核心概念是真的有价值的。你们来回消息到凌晨四点，把那团混沌一点点捋成了一根看得见方向的线。第二天林小卷顶着两个硬币大的黑眼圈出现，说"老师你也没睡？" 你们对视了两秒，同时伸手按下了咖啡机。',
          effects: [
  ...
  lxj_conference_abstract: {
    ...
    description: [
      ...
      '四点五十八分，摘要准时弹进收件箱。你打开一读：语言简洁有力，核心问题一针见血，亮点段落甚至带着一点文学质感，像是有人打磨了整整一周。审稿人回评："表达清晰，立意新颖。" 二十八分钟写出来的东西。',
    ],
  ...
  lxj_fake_progress: {
    ...
    description: [
      '林小卷在月度汇报里宣布项目进度已达百分之七十，语气四平八稳，PPT做得图文并茂，配色甚至挺好看。你听着听着，发现几个关键实验只展示了结果截图，方法细节全程蒸发。',
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你什么都没说，只把代码仓库链接发给他，备注"有空看看"。林小卷沉默了整整三分钟——然后仓库出现了一次新提交：填上了两个 `pass`，把 `TODO_FIXME_URGENT` 悄悄改成了 `TODO_IN_PROGRESS`。进步微小，但至少是真实发生过的。',
          effects: [
      ...
      },
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你把代码截图发过去，问"这三个 pass 怎么回事"。沉默了整整五分钟，林小卷发来消息："老师，实际上……大概四十五？" 后面跟了一个苦笑表情，然后："我这周搞定两个，你给我一周。" 一周后，仓库出现了两次提交，进度单数字更新，这次是真实的——人在被戳穿之后往往比正常状态跑得更快。',
          effects: [
            { type: 'student', studentId: 'lin_xiaojuan', stat: 'favor', delta: -8 },
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你拉了把椅子，什么都没问，只是说"最近怎么样"。林小卷沉默了两秒，然后开口——说了一刻钟，从项目卡死的地方说到对自己的怀疑，再说到一个他从来没跟人讲过的想法。说完他缓了口气，把那张白纸翻过来推给你——上面写着：我到底适不适合做科研。你们又聊了半小时。你离开的时候，那杯凉咖啡已经不见了，换成了一杯热的。',
          effects: [
            { type: 'student', studentId: 'lin_xiaojuan', stat: 'favor', delta: 8 },
            { type: 'student', studentId: 'lin_xiaojuan', stat: 'happiness', delta: 8 },
            { type: 'lab', stat: 'energy', delta: -5 },
  ...
  lxj_sudden_expert: {
    ...
    description: [
      ...
      '他指出了组里一直在用的理论框架里的一个裂缝：在某个边界条件下，核心假设会悄悄失效，而所有现有实验恰好全部绕开了那个边界，所以没有一个人发现过。他讲了整整半小时，逻辑环环相扣，你中途插了两个问题，他都答上来了，没有丝毫停顿。',
    ...
    options: [
      ...
      },
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你叫他先跑一套专门设计的测试案例，把边界失效的情况坐实。林小卷花了三天构造了一批反例——漏洞不仅被确认，还比他最初描述的更普遍，覆盖了更多情形。这让文章从"发现问题"升级成"证明问题并给出修复方向"，分量重了不止一个量级。他兴奋地说"老师这比我想的还大"，白板上的字又多了一圈。',
          effects: [
            { type: 'student', studentId: 'lin_xiaojuan', stat: 'favor', delta: 8 },
  ...
  lxj_graduation_panic: {
    ...
    description: [
      ...
      '你注意到备忘录的创建时间——今天下午三点十七分，距离现在四十分钟。他一听说你要约谈，立刻跑去写了这份计划。',
    ],
    prompt: '林小卷距毕业论文只剩一个月，计划刚创建四十分钟，你选择：',
    triggerConditions: [
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你拍拍他的肩膀："好，按计划来，每周给我看一次进展。" 接下来四周，他每周准时出现，带来的章节每次都比你预期多，质量也过得去。最后一周连续三天睡眠不足四小时，但论文准时提交，还比格式要求多了二十页。答辩委员会说"内容扎实"，林小卷回来转告你，表情高度克制，但脚在地板上悄悄踩了一个小舞步，以为你没看见。',
          effects: [
  ...
  lxj_alumni_visit: {
    ...
    description: [
      ...
      '邮件最后，他问你有没有兴趣合作一个行业项目，预算到位，方向也契合。他附上了初步方案的文档链接。文档创建时间：昨晚十一点五十八分——也就是今天发这封邮件之前两分钟写完的。有些人，真的永远不会变。',
    ],
    ...
    options: [
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你回复说可以，问他什么时候方便细聊。三分钟后他回来了，附上视频会议链接，时间定在明天早上九点——你高度怀疑那个链接在邮件发出之前就已经创建好了。合作正式启动，实验室拿到一笔行业资金。你看着他名字出现在合作协议上，想起那个端着奶茶迟到十五分钟进组会的新人，觉得时间这件事有时候挺有意思的。',
          effects: [
            { type: 'lab', stat: 'reputation', delta: 3 },
      ...
      },
      {
        ...
        outcomes: [{
          weight: 1,
          narrative: '你礼貌地说组里最近任务比较满，暂时接不了新合作。林小卷回复"好的老师，有机会再说"，然后附了一句："组里最近在忙什么方向，我顺便帮你留意一下人。" 你盯着这条消息看了几秒——他毕业不到一年，已经在给你介绍人了。你把邮件存进"日后跟进"文件夹，想着那个总说"快了快了"的学生，现在确实快了。',
          effects: [
